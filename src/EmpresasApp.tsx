...
export default function EmpresasApp({ entityToEdit, clearEntityToEdit }: EmpresasAppProps) {
  ...
  const [isUploadingCNPJ, setIsUploadingCNPJ] = useState(false);
  const [cnpjUploadMessage, setCnpjUploadMessage] = useState('');
  const fileInputRef = useRef<HTMLInputElement>(null);

  const handleUploadCartaoCNPJ = async (e: React.ChangeEvent<HTMLInputElement>) => {
    const file = e.target.files?.[0];
    if (!file) return;

    setIsUploadingCNPJ(true);
    setCnpjUploadMessage('Extraindo dados...');
    ...
    try {
      ...
      reader.onload = async (event) => {
        ...
        if (extracted) {
           setEditingEmpresa(prev => ({
             ...prev,
             nome: extracted.razaoSocial || prev?.nome || '',
             cnpj: extracted.cnpj || prev?.cnpj || '',
             telefone: extracted.telefone || prev?.telefone || '',
             email: extracted.email || prev?.email || '',
             enderecoCompleto: extracted.enderecoCompleto || prev?.enderecoCompleto || ''
           }));
           setCnpjUploadMessage('Dados extraídos com sucesso!');
           setTimeout(() => setCnpjUploadMessage(''), 3000);
        } else {
      ...
      console.error(err);
      setCnpjUploadMessage('Erro: ' + err.message);
      setTimeout(() => setCnpjUploadMessage(''), 3000);
    } finally {
      setIsUploadingCNPJ(false);
    }
  };
  ...
  const openEmpresaModal = (empresa?: Empresa) => {
    if (empresa) {
      ...
      setEditingEmpresa({ nome: '', cnpj: '', codigo: '', sindicatoId: '', telefone: '', email: '', enderecoCompleto: '' });
    }
    setIsModalOpen(true);
  };
      ...
      {isModalOpen && activeTab === 'EMPRESAS' && editingEmpresa && (
        <div className="fixed inset-0 bg-black/60 z-50 flex items-center justify-center p-4">
          <div className="bg-slate-900 border border-slate-700/50 rounded-2xl w-full max-w-md shadow-2xl overflow-hidden">
            ...
            <form onSubmit={handleSaveEmpresa} className="p-6 space-y-4 max-h-[80vh] overflow-y-auto custom-scrollbar">
              
              {/* Área de Upload do Cartão CNPJ */}
              <div className="mb-4">
                <label className="block text-xs font-bold text-slate-500 uppercase tracking-widest mb-2">Importar Cartão CNPJ (Auto-preenchimento)</label>
                <div 
                  className={`border-2 border-dashed rounded-xl p-4 text-center transition-colors cursor-pointer relative ${isUploadingCNPJ ? 'border-indigo-500/50 bg-indigo-500/5' : 'border-slate-700 hover:border-indigo-500/50 hover:bg-slate-800/50'}`}
                  onDragOver={(e) => e.preventDefault()}
                  onDrop={(e) => {
                    e.preventDefault();
                    if (e.dataTransfer.files && e.dataTransfer.files[0]) {
                      const fakeEvent = { target: { files: e.dataTransfer.files } } as any;
                      handleUploadCartaoCNPJ(fakeEvent);
                    }
                  }}
                ...
                >
                  <input 
                    ...
                    className="hidden" 
                    accept="image/*,.pdf" 
                    onChange={handleUploadCartaoCNPJ} 
                  />
                  {isUploadingCNPJ ? (
                    <div className="flex flex-col items-center justify-center space-y-2 text-indigo-400">
                      <RefreshCw className="w-6 h-6 animate-spin" />
                      <span className="text-sm font-medium">{cnpjUploadMessage}</span>
                    </div>
                  ) : (
                    <div className="flex flex-col items-center justify-center space-y-2 text-slate-400">
                      <Upload className="w-6 h-6 mb-1 text-slate-500" />
                      <span className="text-sm font-medium text-slate-300">Arraste ou clique para enviar Cartão CNPJ</span>
                      <span className="text-xs text-slate-500">PDF ou Imagem para extração com IA</span>
                    </div>
                  )}
                  {cnpjUploadMessage && !isUploadingCNPJ && (
                    <div className="absolute inset-0 bg-emerald-500/10 border border-emerald-500/20 rounded-xl flex items-center justify-center text-emerald-400 text-sm font-medium animate-in fade-in">
